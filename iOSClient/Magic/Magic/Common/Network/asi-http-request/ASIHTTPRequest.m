...
- (void)startRequest
{
    ...
    [self setContentDisposition:nil];
...
//    NSData *d = [NSMakeCollectable(CFHTTPMessageCopySerializedMessage(message)) autorelease];
//    NSString* string = [[[NSString alloc] initWithBytes:[d bytes] length:[d length] encoding:NSUTF8StringEncoding] autorelease];
//    NSLog(@"response header data = %@\n, header = %@", string, [self.responseHeaders description]);
//    if ([string length] > 0){
//        NSRange range = [string rangeOfString:@"Content-Disposition"];
...
//            self.contentDisposition = [string substringWithRange:range];
//            NSLog(@"content disposition = %@", self.contentDisposition);
//        }
//    }    
