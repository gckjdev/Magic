...
#define kSelectFromAlbum    @"从相册选择"
#define kTakePhoto          @"拍照"
#define kCancel             @"取消"
#define kResetDefault       @"重置"

@interface ChangeAvatar () {
    NSInteger _buttonIndexAlbum;
    NSInteger _buttonIndexCamera;
    NSInteger _buttonIndexReset;
}
...
//    [actionSheet showInView:[superViewController view]];
    [actionSheet showInView:superViewController.rdv_tabBarController.tabBar];
//    [actionSheet release];
}
        ...
        userOriginalImage:(BOOL)userOriginalImage
{
    ...
    NSInteger index = [sheet addButtonWithTitle:NSLS(kCancel)];
    ...
    [sheet showInView:superViewController.rdv_tabBarController.tabBar];
...
{
    __block UIImage *image = [info objectForKey:self.userOriginalImage?UIImagePickerControllerOriginalImage:UIImagePickerControllerEditedImage];
...
- (void)handleSelectAvatar:(NSInteger)buttonIndex
{
    if (buttonIndex == _buttonIndexAlbum) {
        [self selectPhoto];
    }
